...
def preview(request):
    ...
    if resp["response_type"].lower() == "json":
        data = resp["data"] if (isinstance(resp["data"], dict) or isinstance(resp["data"], list)) else json.loads(
            resp["data"])
    ...
    if resp["response_type"].lower() == "csv":
        ...
        data = (data if len(data) < 50 else data.head(50)) if res_type == "api" else data.head(
            int(row_count) if row_count != None else 0)
...
def fetchapi(resource_id, api_data_params):
    ...
    if res_type == "api":
        ...
        try:
            ...
            context = {
                "Success": True,
                "data": response["data"] if response["response_type"].lower() != "csv" else pd.DataFrame(
                    response["data"]),
                # pd.read_csv(StringIO(response["data"]) ),
